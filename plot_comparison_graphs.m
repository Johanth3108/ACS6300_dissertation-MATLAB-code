...
figure;
subplot(2,1,1);
plot(timempc, rad2deg(Xmpc_with(:, 2)), 'm', 'LineWidth', 1.5);
xlabel('Time (s)');
...
plot(timempc, rad2deg(refmpc_with(:, 2)), 'g--', 'LineWidth', 1.5);
legend('MPC controller output', 'LQR controller output', 'reference');
grid on;

subplot(2,1,2);
plot(timempc, rad2deg(Xmpc_with(:, 1)), 'r', 'LineWidth', 1.5);
xlabel('Time (s)');
ylabel('\theta (degrees)');
...
plot(timempc, rad2deg(refmpc_with(:, 1)), 'g--', 'LineWidth', 1.5);
legend('MPC controller output', 'LQR controller output', 'reference');
grid on;
